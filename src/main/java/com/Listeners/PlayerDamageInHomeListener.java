      ...
      if (p != null) {
         if (Util.CheckIsHome(p.getWorld().getName().replace(Variable.world_prefix, ""))) {
            if (!Util.Check(p, p.getWorld().getName().replace(Variable.world_prefix, ""))) {
               if (Variable.bungee) {
                  if (MySQL.getPVP(p.getWorld().getName().replace(Variable.world_prefix, "")).equalsIgnoreCase("false")) {
                     String temp = Variable.Lang_YML.getString("NoPermissionPVP");
                     p.sendMessage(temp);
                     event.setCancelled(true);
                  }
               } else {
                  File f2 = new File(Variable.Tempf, event.getDamager().getWorld().getName().replace(Variable.world_prefix, "") + ".yml");
