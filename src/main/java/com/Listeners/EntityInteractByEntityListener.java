      ...
      if (!event.isCancelled()) {
         if (event.getRightClicked().getType() != EntityType.PLAYER) {
            if (Util.CheckIsHome(event.getPlayer().getWorld().getName())) {
               if (!Util.Check(event.getPlayer(), event.getPlayer().getWorld().getName()) && Main.JavaPlugin.getConfig().getBoolean("EnableEntityInteract")) {
                  event.getPlayer().sendMessage(Variable.Lang_YML.getString("ProtectEntity"));
                  event.setCancelled(true);
                  if (event.getRightClicked().getType().toString().toUpperCase().contains("BLADESTAND")
                     && event.getPlayer().getItemInHand().getType().toString().equalsIgnoreCase("AIR")) {
                  ...
                  if (this.list.contains(event.getPlayer().getName())) {
                     ...
                     Bukkit.dispatchCommand(event.getPlayer(), "sh gift send " + event.getPlayer().getWorld().getName());
                     this.list.remove(event.getPlayer().getName());
                  }
               }
            }
